 ...
 ******************************************************************************/
void
UsbCore::enableGlobalInterrupt(void) const {
...
UsbCore::enableInterrupt(const Interrupt_t p_irq) const {
    ...
    for (const UsbCore::irq_handler_t *cur = this->m_irq_handler; !found && cur->m_irq != 0; cur++) {
        if (cur->m_irq == p_irq) {
            found = true;
        }
    }
    
    for (const UsbCore::irq_handler_t *cur = this->m_otgirq_handler; !found && cur->m_irq != 0; cur++) {
        if (cur->m_irq == p_irq) {
            found = true;
        }
    }
    ...
    if (!found) {
        found = this->m_usbDevice->isIrqHandled(p_irq);
    }
 ...
 ******************************************************************************/
void
UsbCore::disableGlobalInterrupt(void) const {
    this->m_usbCore->GAHBCFG &= ~USB_OTG_GAHBCFG_GINT;
}
...
UsbCore::setupMode(const DeviceMode_e p_mode) {
    ...
    this->enableInterrupt(e_ModeMismatch);
}
 ...
 ******************************************************************************/
void
UsbCore::handleSessionEnd() const {
    /* TODO */
}
